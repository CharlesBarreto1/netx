...
import { hrApi, type PayrollReport } from '@/lib/hr-api';
...
export default function HrReportsPage() {
  const t = useTranslations('hr.reports');
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Stat label={t('totals.employees')} value={String(data.totals.employees)} />
            <Stat label={t('totals.totalNet')} value={`R$ ${data.totals.totalNet.toFixed(2)}`} />
            <Stat label={t('totals.totalPaid')} value={`R$ ${data.totals.totalPaid.toFixed(2)}`} accent="text-green-600" />
            <Stat label={t('totals.totalPending')} value={`R$ ${data.totals.totalPending.toFixed(2)}`} accent="text-amber-600" />
          </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {data.rows.map((r) => (
                  <tr key={r.employeeId} className="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{statusLabel(r.status)}</td>
